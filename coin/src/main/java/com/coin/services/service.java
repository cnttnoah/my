...
public class service{
	...
	public int Service(int num, parameter Par) {
		if(Par.getBtn().equals("+")){num++;}
		if(Par.getBtn().equals("-")&&num>1) {num--;}	// POST된 버튼의 종류에 따라 K값 증감
		
		if(Par.getBtn().equals("계산")) {
			if(Par.getPrice()==""||Par.getPrice()==null){// T 유효성 체크
				t=Par.getPrice();
				Par.setStr("입력이 유효하지 않습니다."); // jsp에서 출력 할 메세지
				return num;
			}
			...
			Par.setStr("총 "+Par.getResult()+"가지<br>"+Par.getStr());
			num=1;
		}
		return num;
	}
